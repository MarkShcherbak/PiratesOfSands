...
namespace IndentSurface
{
    public class SnowTraceCtrl : MonoBehaviour
    {
        ...
        private void Start()
        {
            ShipModelView[] listShipMV = FindObjectsOfType<ShipModelView>();
            int j = 0;
            foreach (ShipModelView item in listShipMV)
            {
                actors.Add(item.gameObject);
                j++;
            }
            ...
            posCamera.depth = Camera.allCameras[0].depth - 2; // first
            posCamera.targetTexture = rtWorldPos;

            posToNormalCamera.depth = Camera.allCameras[0].depth - 1; // second
        ...
        {
            camera.transform.position = Camera.allCameras[0].transform.position;
            camera.transform.rotation = Camera.allCameras[0].transform.rotation;
            camera.fieldOfView = Camera.allCameras[0].fieldOfView;
            camera.nearClipPlane = Camera.allCameras[0].nearClipPlane;
            camera.farClipPlane = Camera.allCameras[0].farClipPlane;
            camera.aspect = Camera.allCameras[0].aspect;
        }
